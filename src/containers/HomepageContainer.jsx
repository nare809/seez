...
class HomepageContainer extends Component {
  ...
  componentDidMount() {
    ...
    console.log("Current language in HomepageContainer:", currentLang);
    
    // Use language-specific API for Telugu and other Indian languages
    if (currentLang === 'te') {
      console.log("Loading Telugu-specific movies");
      getTeluguMovies()
        .then(movies => {
          if (!movies) return;
          this.props.setNowPlayingMovies(movies.results.splice(0, 20));
          this.setState({ loadingMovies: false });
        }).catch((e) => { 
          console.error("Error loading Telugu movies:", e);
          this.setState({ error: true, loadingMovies: false }); 
        });
    } else if (['hi', 'ta', 'ml', 'kn', 'bn', 'mr', 'gu'].includes(currentLang)) {
      console.log(`Loading ${currentLang}-specific movies`);
      getMoviesInUserLanguage()
        .then(movies => {
          if (!movies) return;
          this.props.setNowPlayingMovies(movies.results.splice(0, 20));
          ...
          console.error(`Error loading ${currentLang} movies:`, e);
          this.setState({ error: true, loadingMovies: false }); 
        });
    } else {
      // Use regular API for other languages
      getNowPlayingMovies()
        .then(movies => {
          if (!movies) return;
